    ...
    public class AccountController : Controller
    {
        ...
        public IActionResult Login(AccountLoginModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                try
                {
                    ...
                    var ususarioRepository = new UsuarioRepository();
                    var usuario = ususarioRepository.GetByEmailAndSenha(model.Email, model.Senha);
        ...
        public IActionResult PasswordRecover(AccountPasswordRecoverModel model)
        {
            if(ModelState.IsValid)
            {
                try
                {
                    //buscar usuário no banco de dados através do email informado
                    var usuarioRepository = new UsuarioRepository();
                    ...
                    if (usuario != null)
                    {
                        //FALTA IMPLEMENTAR O ENVIO DE EMAIL!
                        TempData["Mensagem"] = $"Olá {usuario.Nome}, você receberá um email para cadastrar nova senha.";
                    }
                    else
                    {
